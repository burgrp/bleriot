...
| **Instance** | One physical device: its name, address, key, channel, device type, optional Registry name mappings, and config. Describes *what a node is and where it is*. | Hand-authored Go (the site inventory) | n/a |
...
instance (Go)                         │
 (address, key, channel, names, config) ─┘
```
...
By default, the register name is scoped by the device instance name when
published to the Registry: instance `kitchen` + register `temperature` →
`kitchen.temperature`. This keeps names distinct across devices that share one
device type.

An instance may replace selected defaults with arbitrary complete Registry
names through `inventory.RegistryNames`, a defined `map[uint16]string` type.
Keys are permanent register tags rather than driver names, so a mapping remains
attached to the same wire register if the driver-facing name changes. Omitted
tags retain the default name; a mapping replaces the default rather than
publishing an alias:

```go
inventory.Instance{
  Name: "ssr5a",
  Type: ssr5a.Type(config),
  RegistryNames: inventory.RegistryNames{
    ssr5a.RegChannel1: "zones.kitchen.valves.radiator",
  },
}
```

Remapping changes only the Registry-facing name. The original register name,
tag, type, conversion, access mode, polling, and wire `GET`/`SET` identity are
unchanged. Inventory validation rejects mappings with an unknown tag or empty
destination and rejects duplicate final Registry names across the complete
inventory, including collisions between mapped and default names.
...
| Field         | Type             | Description                                                       |
|---------------|------------------|-------------------------------------------------------------------|
| name          | string           | Device name, unique within the inventory; scopes default register names |
...
| registryNames | `RegistryNames`  | Optional register-tag → complete Registry-name replacements       |
...
each device type, instance names and nonzero addresses are unique, each channel
uses a single spreading factor, and every resolved Registry name is non-empty,
