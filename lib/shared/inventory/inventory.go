...
type DeviceType struct {
	...
	Registers []Register
	// Chip is the MCU the device's firmware runs on. It tells the build tooling
	// how to build the image (TinyGo target) and flash it (pyocd target and CMSIS
	// pack). The running hub ignores it.
	Chip Chip
}

// Chip describes how to build and flash firmware for an MCU type. It is a
// firmware/hardware fact of a device type, not a per-device deployment fact, so
// it lives on the DeviceType. Predefined Puya PY32 chips are provided by the
// puya package (e.g. puya.PY32F030x8); a site can also declare its own.
type Chip struct {
	// Name identifies the chip in errors, e.g. "py32f030x8".
	Name string
	// TinygoTarget is the tinygo --target name used to build the firmware image,
	// e.g. "py32f030x8".
	TinygoTarget string
	// PyocdTarget is the pyocd target name used to flash the image, e.g.
	// "py32f030x8".
	PyocdTarget string
...
type Instance struct {
	...
	Name string
	// Address is the node's random, nonzero RF address. It is generated by the
	// "new" command and baked into the firmware image by "make".
...
func (inv Inventory) Validate() error {
	...
	for i, inst := range inv {
		...
		if inst.Address == ([config.AddrLen]byte{}) {
			return fmt.Errorf("instance %q: address 0 is reserved", inst.Name)
		}
		if owner, ok := seenAddress[inst.Address]; ok {
			return fmt.Errorf("instances %q and %q have duplicate address %X", owner, inst.Name, inst.Address)
		}
