...
func TestParsePorts(t *testing.T) {
	...
	// The "," separator is unambiguous for by-path devices that contain colons.
	byPath := "/dev/serial/by-path/pci-0000:07:00.4-usb-0:2.1:1.0"
	ports, err = parsePorts([]string{byPath + ",37"})
	if err != nil {
		t.Fatalf("parsePorts by-path: %v", err)
	}
	if len(ports) != 1 || ports[0].device != byPath || ports[0].channel != 37 {
		t.Fatalf("by-path parse = %+v, want device %q channel 37", ports, byPath)
	}

	// Colons in the device path are kept verbatim; only "," separates the channel.
	ports, err = parsePorts([]string{"/dev/ttyUSB0:not-a-channel,5"})
	if err != nil {
		t.Fatalf("parsePorts colon device: %v", err)
	}
	if ports[0].device != "/dev/ttyUSB0:not-a-channel" || ports[0].channel != 5 {
		t.Fatalf("colon device parse = %+v", ports)
	}

	for _, bad := range []string{"nochannel", "/dev/ttyUSB0:37", "/dev/ttyUSB0,", ",37", "/dev/ttyUSB0,999"} {
